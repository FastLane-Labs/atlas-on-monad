...
contract Sorter is AtlasConstants {
    ...
    function _verifySolverEligibility(
        ...
        returns (bool)
    {
        // Make sure the solver has enough funds committed
        uint256 solverBalance = SHMONAD.convertToAssets(SHMONAD.balanceOfCommitted(POLICY_ID, solverOp.from));
        ...
        // Solvers can only do one tx per block - this prevents double counting committed balances
