...
contract AtlasVerification is EIP712, NonceManager, DAppIntegration {
        ...
        )
    {
        ...
        if (dConfig.callConfig.checkMetacallGasLimit()) {
            ...
            // If gas limit is too low, the committed balance threshold checked may not cover all gas reimbursements.
            if (metacallGasLeft < metacallExecutionGas - _execGasLowerTolerance) {
                verifyCallsResult = ValidCallsResult.MetacallGasLimitTooLow;
            }
            // If gas limit is too high, the committed balance threshold checked could unexpectedly price out solvers.
