...
abstract contract GasAccounting is SafetyLocks {
    ...
    /// (msg.value) or by approving Atlas to use a certain amount of the solver's committed shMON.
    /// @param maxApprovedGasSpend The maximum amount of MON from the solver's committed shMON that Atlas can deduct to
    ...
    function reconcile(uint256 maxApprovedGasSpend) external payable returns (uint256 owed) {
        ...
        // will be subtracted from the solver's committed shMON during _settle().
        ...
        uint256 _committedBalance = SHMONAD.convertToAssets(SHMONAD.balanceOfCommitted(POLICY_ID, _currentSolver));

        // Solver can only approve up to their committed balance, not more
        if (maxApprovedGasSpend > _committedBalance) maxApprovedGasSpend = _committedBalance;
        ...
        // - gas liabilities must be repaid (using committed shMON or native token)
        ...
        if (_borrows > _repays) {
            ...
            // paid by the solver's committed shMON balance.
            uint256 _excess;
    ...
    /// pays from their committed shMON (shares), and Atlas recieves the specified amount of MON (native token).
    ...
    /// @param account The address of the committed shMON account from which MON is taken.
    ...
    function _assign(address account, uint256 amount) internal returns (uint256 deficit) {
        ...
        SHMONAD.agentWithdrawFromCommitted(POLICY_ID, account, address(this), amount, 0, true);
    }

    /// @notice Deposits MON, and commits resulting shMON on behalf of an account, in ShMonad.
    /// @param account The address of the recipient of the committed shMON.
    /// @param amount The amount of MON by which to increase the account's committed balance (will be converted to
    /// shMON).
    function _credit(address account, uint256 amount) internal {
        ...
        SHMONAD.depositAndCommit{ value: amount }(POLICY_ID, account, type(uint256).max);
    }

    /// @notice Accounts for the gas cost of a failed SolverOperation, either by increasing writeoffs (if the bundler is
    /// blamed for the failure) or by assigning the gas cost to the solver's committed shMON balance (if the solver is
        ...
        internal
    {
        ...
        if (result.bundlersFault()) {
            ...
            // In `_assign()`, the solver's committed shMON balance is reduced by `_gasValueWithSurcharges`. Any deficit
    ...
    ///      - If solver fault: Attempts to charge the solver's committed shMON using `_assign` for the gas cost (no
        ...
        returns (uint256 claimsPaidToBundler, uint256 netAtlasGasSurcharge)
    {
        ...
        // Borrows can only be repaid in native token, not from committed balances.
        ...
        if (ctx.solverSuccessful) {
            ...
            uint256 _deficit; // Any shortfall that the winning solver is not able to repay from committed balance
        ...
        // TODO this wont work - solvers pay gas using bonded shMON. So no native assets to send here.
        // Atlas gas surcharge is sent to ShMonad as yield for all shMON holders.

        // Bundler is treated as the yield originator.
        if (netAtlasGasSurcharge != 0) SHMONAD.boostYield{ value: netAtlasGasSurcharge }(ctx.bundler);
