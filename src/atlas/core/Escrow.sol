...
abstract contract Escrow is AtlETH {
        ...
        returns (uint256, uint256 gasLimit)
    {
        ...
        // Get solver's committed shMON amount (measured in MON terms) to check if enough to pay for execution
        uint256 _solverBalance = SHMONAD.convertToAssets(SHMONAD.balanceOfCommitted(POLICY_ID, solverOp.from));

        // Checks if solver's committed balance is enough to cover the max charge should they win, including surcharges
