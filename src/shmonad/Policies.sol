...
abstract contract Policies is AtomicUnstakePool {
    ...
    /**
     * @inheritdoc IShMonad
     * @dev Directly calls _commitToPolicy with the msg.sender as the source of shares
     */
    function commit(uint64 policyID, address commitRecipient, uint256 shares) external onlyActivePolicy(policyID) {
        _commitToPolicy(policyID, msg.sender, commitRecipient, shares, true);
    }
        ...
        onlyActivePolicy(policyID)
    {
        ...
        _commitToPolicy(policyID, msg.sender, sharesRecipient, sharesToCommit, true);
    }

    /**
     * @inheritdoc IShMonad
     * @dev Uses _requestUncommitFromPolicy to handle the actual uncommitting logic
     */
    ...
    /**
     * @inheritdoc IShMonad
     * @dev Direct wrapper around _completeUncommitFromPolicy with the caller as the beneficiary
     */
    ...
    /**
     * @inheritdoc IShMonad
     * @dev Completes uncommitment then immediately redeems them for the underlying asset (MON)
     */
    ...
    /**
     * @inheritdoc IShMonad
     * @dev Completes uncommitment from one policy and immediately commits to another, maintaining total committed
     * supply
     */
        ...
        onlyActivePolicy(toPolicyID)
    {
        ...
        _commitToPolicy(toPolicyID, msg.sender, sharesRecipient, shares, true);
    }
    ...
    /**
     * @inheritdoc IShMonad
     * @dev Creates a new policy and adds the caller as the first agent
     */
    ...
    // Calculated as: committed - held + uncommitting + remaining top-up allowance
     ...
     * @param changeCommittedTotalSupply Whether to increase the committedTotalSupply
    ...
    function _commitToPolicy(
        ...
        address sharesRecipient,
        uint256 shares,
        bool changeCommittedTotalSupply
    )
        internal
    {
        ...
        unchecked {
            ...
            // Increase committedTotalSupply if requested
            if (changeCommittedTotalSupply) {
        ...
        internal
    {
        ...
        if (fundsAvailable < shares) {
            ...
            uint128 takenFromUncommitting = uint128(Math.min(shares - fundsAvailable, uncommittingData.uncommitting));
        ...
        if (fundsAvailable < shares) {
            uint128 committedShortfall = shares - fundsAvailable;
    ...
    // Returns the amount of new shares committed via top-up. Possible values are:
    // 1) 0 if returned early and no new shares are committed
    // 2) `sharesRequested` + user's `minCommitted` if settings and uncommitted balance allow
    // 3) `sharesRequested` only if not enough uncommitted to get back to user's `minCommitted` level
    function _tryTopUp(
        ...
        returns (uint128)
    {
        ...
        // Then check if remaining top-up limit in period will be exceeded. If so, return early with 0 shares taken.
        if (topUpData.totalPeriodTopUps + sharesRequested > topUpSettings.maxTopUpPerPeriod) return 0;

        // Not enough in uncommitted to top-up by `sharesRequested` amount. Return early with 0 shares taken.
        if (balance.uncommitted < sharesRequested) return 0;

        // Now, determine if we can top up by `sharesRequested` + `minCommitted`, or just `sharesRequested`.
        if (
            balance.uncommitted >= sharesRequested + committedData.minCommitted
                && topUpSettings.maxTopUpPerPeriod
                    >= sharesRequested + committedData.minCommitted + topUpData.totalPeriodTopUps
        ) {
            // If possible, top-up the user's committed balance such that it will be at the `minCommitted` level after
            // the `sharesRequested` amount has been spent.
            sharesCommitted = sharesRequested + committedData.minCommitted;
        } else {
            // If not enough uncommitted to get back to the user's `minCommitted` level, just take `sharesRequested`
            // NOTE: We have already checked above that:
            //   1) the user has enough uncommitted balance to cover `sharesRequested`
            //   2) the user's top-up limit in the period will not be exceeded if topping up be `sharesRequested`
            sharesCommitted = sharesRequested;
        }
