...
These are the ledger balances that ShMonad tracks:
    ...
    LIABILITIES:
        ...
        - commissionPayable: the amount of MON that has been collected and escrowed to fund future LST research
...
library AccountingLib {
    ...
    function totalLiabilities(
        ...
        returns (uint256)
    {
        return liabilities.redemptionsPayable + liabilities.rewardsPayable + admin.commissionPayable;
    }
    ...
    */
    ...
    /// @param admin Admin values containing commissionPayable.
    ...
    function targetAtomicAllocatedAmount(
        WorkingCapital memory workingCapital,
        CurrentLiabilities memory liabilities,
        AdminValues memory admin,
        uint256 nativeTokenBalance,
        uint256 scaledTargetLiquidityPercentage
    )
        internal
        pure
        returns (uint256)
    {
        uint256 _equity = totalEquity(workingCapital, liabilities, admin, nativeTokenBalance);
        return _equity * scaledTargetLiquidityPercentage / SCALE;
    }
    ...
    function targetAtomicAllocatedAmount(
        ...
        returns (uint256)
    {
        return equity * scaledTargetLiquidityPercentage / SCALE;
    }

    function targetAtomicAllocatedDelta(
        WorkingCapital memory workingCapital,
        AtomicCapital memory atomicCapital,
        CurrentLiabilities memory liabilities,
        AdminValues memory admin,
        ...
        internal
        pure
        returns (uint256 deltaAmount, bool increase)
    {
        uint256 _targetAtomicAllocatedAmount = targetAtomicAllocatedAmount(
            workingCapital, liabilities, admin, nativeTokenBalance, scaledTargetLiquidityPercentage
        );
        uint256 _currentAtomicAllocatedAmount = atomicCapital.allocatedAmount;

        // Calculates the absolute distance between target and current
        deltaAmount = Math.dist(_targetAtomicAllocatedAmount, _currentAtomicAllocatedAmount);

        // Indicates whether an increase is required for current to reach target
        increase = _targetAtomicAllocatedAmount > _currentAtomicAllocatedAmount;
    }

    function targetStakedAmount(
        WorkingCapital memory workingCapital,
        CurrentLiabilities memory liabilities,
        AdminValues memory admin,
        uint256 nativeTokenBalance,
    ...
    {
        uint256 _equity = totalEquity(workingCapital, liabilities, admin, nativeTokenBalance);
        uint256 _targetAtomicAllocatedAmount = targetAtomicAllocatedAmount(_equity, scaledTargetLiquidityPercentage);
        uint256 _reservedAssets = workingCapital.reservedAmount;
        // uint256 _currentAssets = 0; // Assign stake targets assuming that current assets are zero
        uint256 _unstakedAssets = _targetAtomicAllocatedAmount + _reservedAssets;
        return _saturatingSub(_equity, _unstakedAssets);

        /*
        NOTE: the target stake amount for epoch n+1 should also be very close to this formula. 

        s_globalEpoch_n+1.targetStakeAmount =
                s_globalEpoch_n.targetStakeAmount  
        +   (1 - targetAllocatedPercent) * (s_globalCashFlows_n.queueToStake - s_globalCashFlows_n.queueForUnstake)
        
        */
    }

    function targetStakedDelta(
        WorkingCapital memory workingCapital,
        CurrentLiabilities memory liabilities,
        AdminValues memory admin,
        uint256 nativeTokenBalance,
        ...
        returns (uint256 deltaAmount, bool increase)
    {
        uint256 _currentStakedAmount = workingCapital.stakedAmount;
        uint256 _targetStakedAmount =
            targetStakedAmount(workingCapital, liabilities, admin, nativeTokenBalance, scaledTargetLiquidityPercentage);

        // Calculates the absolute distance between target and current
        deltaAmount = Math.dist(_targetStakedAmount, _currentStakedAmount);

        // Indicates whether an increase is required for current to reach target
        increase = _targetStakedAmount > _currentStakedAmount;
    }
        ...
        returns (uint256)
    {
        ...
        // thereby
        // releasing the allocated capital to be available for non-atomic redemptions
        uint256 _illiquidAmount =
            globalPending.pendingStaking + globalPending.pendingUnstaking + atomicAssets(atomicCapital);
