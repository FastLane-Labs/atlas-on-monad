...
abstract contract ShMonadErrors {
    ...
    // CapitalAllocator
    error ZeroAddress();
    error OnlyMonadStakingContract();
    ...
    error ValidatorCoinbaseAlreadyMapped(address coinbase);
    error InvalidValidatorId(uint256 validatorId);
    error InvalidValidatorAddress(address coinbase);
    error AuthAddressMismatch(address caller, address expectedAuthAddress);
    ...
    error AlreadySyncedWithMonadEpoch(uint64 currentShMonadEpoch, uint64 currentMonadEpoch);
    error ValidatorAlreadyCranked(address coinbase);
    error PlaceholderMustBeCranked();
    error ContractMustBeInitializedWithBalance();
    error InsufficientBalanceAtomicUnstakingPool(uint256 requestedAmount, uint256 availableAmount);
    error ExceededMaxAmountRedeemedThisEpoch(uint256 requestedAmount, uint256 maxAvailableAmount); // Please try again
        // in 2 epochs.
    ...
    error InsufficientPoolLiquidityAfterFee(uint256 requestedAfterFee, uint256 availableAfterFee);
    error TargetLiquidityPercentageExceedsMax();
    error BaseFeeRateExceedsRay(uint256 requestedBaseRate);
    error SlopeFeeRateExceedsRay(uint256 requestedSlopeRate);
    error FeeAffineBaseRateExceedsRay();
    error FeeAffineFullUtilizationExceedsRay();
    error FeeAffineTargetExceedsCapacity();
    ...
    error IneligibleToSelfRegister(uint64 currentEpoch, uint64 lastKnownEpoch);
    error ErrorOnGlobalCrank();
    error ErrorOnValidatorCrank();
