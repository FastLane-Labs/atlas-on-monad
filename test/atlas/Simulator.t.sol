...
contract SimulatorTest is AtlasBaseTest {
    ...
    function test_Simulator_estimateMetacallGasLimit() public view {
    ...
    function test_Simulator_estimateMaxSolverWinGasCharge() public view {
        UserOperation memory userOp = _createUserOp(userEOA);
        bytes32 userOpHash = atlasVerification.getUserOperationHash(userOp);
    ...
    function testFuzz_Simulator_estimateMetacallGasLimit(
        uint32 userGas,
        uint32 solverGas,
        uint8 solverCount
    ) public view {
    ...
    function test_Simulator_withdrawETH_Unauthorized() public {
        // Send some ETH to simulator
        deal(address(simulator), 10 ether);
