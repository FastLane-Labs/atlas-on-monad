...
contract SorterTest is AtlasBaseTest {
    ...
    function setUp() public override {
        ...
        // Bond solver balances using depositAndCommit
        vm.prank(solver1);
        shMonad.depositAndCommit{value: 10 ether}(atlas.POLICY_ID(), solver1, type(uint256).max);
        
        vm.prank(solver2);
        shMonad.depositAndCommit{value: 10 ether}(atlas.POLICY_ID(), solver2, type(uint256).max);
        
        vm.prank(solver3);
        shMonad.depositAndCommit{value: 10 ether}(atlas.POLICY_ID(), solver3, type(uint256).max);
        
        vm.prank(solver4);
        shMonad.depositAndCommit{value: 10 ether}(atlas.POLICY_ID(), solver4, type(uint256).max);
    }
    
    function test_Sorter_sortBids_EmptyArray() public view {
    ...
    function test_Sorter_sortBids_SingleValidSolver() public view {
        UserOperation memory userOp = _createUserOp(userEOA, address(dappControl));
    ...
    function test_Sorter_sortBids_MultipleSolversDescending() public view {
        UserOperation memory userOp = _createUserOp(userEOA, address(dappControl));
        bytes32 userOpHash = atlasVerification.getUserOperationHash(userOp);
    ...
    function test_Sorter_sortBids_InvalidSolverFiltered() public view {
        UserOperation memory userOp = _createUserOp(userEOA, address(dappControl));
        bytes32 userOpHash = atlasVerification.getUserOperationHash(userOp);
    ...
    function test_Sorter_sortBids_InsufficientBalance() public {
        UserOperation memory userOp = _createUserOp(userEOA, address(dappControl));
        bytes32 userOpHash = atlasVerification.getUserOperationHash(userOp);
        ...
        vm.prank(poorSolver);
        shMonad.depositAndCommit{value: 0.01 ether}(atlas.POLICY_ID(), poorSolver, type(uint256).max);
    ...
    function test_Sorter_sortBids_WrongBidToken() public view {
        UserOperation memory userOp = _createUserOp(userEOA, address(dappControl));
        bytes32 userOpHash = atlasVerification.getUserOperationHash(userOp);
    ...
    function test_Sorter_sortBids_ExpiredDeadline() public {
        UserOperation memory userOp = _createUserOp(userEOA, address(dappControl));
        bytes32 userOpHash = atlasVerification.getUserOperationHash(userOp);
        ...
        // First advance the block relative to the current height so the deadline can be in the past.
        vm.roll(block.number + 10);
        uint256 expiredDeadline = block.number - 1;
        
        solverOps[1] = _createSolverOp(solver2, userOpHash, 2 ether, address(dappControl));
        solverOps[1].deadline = expiredDeadline; // Expired relative to current block
    ...
    function test_Sorter_sortBids_SolverAlreadyActive() public {
    ...
    function test_Sorter_sortBids_LowMaxFeePerGas() public view {
        UserOperation memory userOp = _createUserOp(userEOA, address(dappControl));
    ...
    function test_Sorter_sortBids_EqualBids() public view {
        UserOperation memory userOp = _createUserOp(userEOA, address(dappControl));
        bytes32 userOpHash = atlasVerification.getUserOperationHash(userOp);
    ...
    function testFuzz_Sorter_sortBidsRandomAmounts(
        uint256 bid1,
        uint256 bid2,
        uint256 bid3,
        uint256 bid4
