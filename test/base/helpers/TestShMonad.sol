...
import { UNKNOWN_VAL_ID, UNKNOWN_VAL_ADDRESS } from "../../../src/shmonad/Constants.sol";
...
contract TestShMonad is ShMonad {
    ...
    function exposeGlobalAssetsLast() external view returns (uint120 debits, uint120 credits) {
        return this.getGlobalCashFlowsCurrent(-1);
    }

    function exposeGlobalRewardsCurrent() external view returns (uint120 rewardsPayable, uint120 earnedRevenue) {
        return this.getGlobalRewardsCurrent(0);
    }
    ...
    function setEligibilityEpoch(uint64 valId, uint64 epoch) external {
        s_valEligibility[valId] = epoch;
    }
    ...
    function harnessInitiateStakeAllocationDecrease(
        address coinbase,
        ...
        returns (uint128, uint128)
    {
        uint64 valId = _validatorIdForCoinbase(coinbase);
        ValidatorData memory data = _getValidatorData(valId);
        return _initiateStakeAllocationDecrease(data.coinbase, data.id, nextTargetStakeAmount, netAmount);
    }

    function harnessInitiateStakeAllocationIncrease(
        address coinbase,
        uint128 nextTargetStakeAmount,
        uint128 netAmount
    )
        ...
        returns (uint128, uint128)
    {
        uint64 valId = _validatorIdForCoinbase(coinbase);
        ValidatorData memory data = _getValidatorData(valId);
        return _initiateStakeAllocationIncrease(data.coinbase, data.id, nextTargetStakeAmount, netAmount);
    }
    ...
    function harnessSettlePastEpochEdges(address coinbase) external {
        uint64 valId = _validatorIdForCoinbase(coinbase);
        ValidatorData memory data = _getValidatorData(valId);
        _settlePastEpochEdges(data.coinbase, data.id);
    }
